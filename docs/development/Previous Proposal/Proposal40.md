...
```text
swift run SciStationCoreTestRunner：通过（包含 P40 wizard preview / target validation / module config generation / safe directory filtering / privacy keyword scan / existing-root preservation 覆盖）
xcodebuild -project Sci-Station.xcodeproj -scheme Sci-Station -destination 'platform=macOS' build：通过（仍有既有 WebKit actor-isolation warning，非 P40 新增阻塞）
get_errors：P40 编辑的 Swift 文件无错误
```

手动测试报告：`docs/development/manual-tests/runs/2026-05-08_P40_WorkspaceCreationWizard.md`

已知问题：

```text
本轮未执行真实 macOS GUI spot check；Empty Workspace / Settings sheet 打开、按钮交互和真实保存面板选择需在本机 App UI 中补测。
P39.5 前置状态：Implementation complete；GUI-only spot check 仍需在 release/对外演示前补跑。
ChatMarkdownWebView / MarkdownPreviewView 仍有既有 WebKit actor-isolation build warning，未纳入 P40 范围。
```

推迟到 P41 的事项：Module settings page、enable/disable UX、pin to sidebar、directory repair UI、project-level module override。

P40 Questions 本轮决议：

```text
1. P40 不再等待 P39.5 代码实现；wizard 入口开放，但交付记录保留 GUI spot check pending。
2. P40 采用单窗口 wizard。
3. P40 只允许 Minimal / Literature Review 两个内置模板；自定义模块组合留给 P41。
4. Code / Theory / Writing 模板以 coming later 形式展示，不可选择，不默认启用。
5. 创建向导包含 privacy / AI setup confirmation，未确认前不能完成。
6. 预览和写入均以 WorkspaceTemplateRegistry / WorkspaceModuleRegistry / WorkspaceCreationWizard resolver 为唯一来源。
```
